...
$(document).ready(function() {
	...
	$(document).on("submit", "#replyWriteForm", function() {
		console.log("#replyContent : " + $("#replyContent").val());
		...
		// 폼 컨틀로에 입력된  데이터를  id=midas&name=홍길동
		var params = $(this).serialize();
		console.log("replyWriteForm submit - params : " + params);
		
		$.ajax({
			url: "videoReplyWrite.ajax",
			type: "post",
			data: params,
			dataType: "json",
			success: function(resultData, status, xhr) {		
				...
				$.each(resultData, function(index, value) {					
					...
					var date = new Date(value.vr_regdate);
					...
					var result = 
						"<tr class='reply_" + value.vr_no + "'>" 
						+ "<td>"
						+ "	<div class='replyUser'>"
						+ "		<span class='member'>" + value.vr_writer + "</span>"
						+ "	</div>"
						+ "	<div class='replyModify'>"
						+ "		<span class='replyDate'>" + strDate + "</span>"
						+ "		<a href='#' class='modifyReply' data-no='" + value.vr_no + "'>"
						+ "			<img src='images/reply_btn_modify.gif' alt='댓글 수정하기'/>"
						+ "		</a>"
						+ "		<a href='#' class='deleteReply' data-no='" + value.vr_no + "'>"
						+ "			<img src='images/reply_btn_delete.gif' alt='댓글 삭제하기'/>"
						+ "		</a>"
						+ "		<a href='javascript:reportReply('div_" + value.vr_no + "');'>"
						+ "			<img src='images/reply_btn_notify.gif' alt='신고하기'/>"
						+ "		</a>"
						+ "	</div>"
						+ "	<div class='replyContent' id='div_" + value.vr_no + "'>"
						+ "		<pre><span>" + value.vr_reply + "</span></pre>"
						+ "	</div>"
						+ "</td>"
					+ "</tr>";
	...
	$(document).on("click", ".deleteReply", function() {	
		...
		var vr_no = $(this).attr("data-no");
		//var writer = $(this).parent().prev().find(".member").text();
		// vr_writer + "님이 작성한 " + 
		var v_no = $("#replyForm input[name=v_no]").val();
		var result = confirm(vr_no +"번 댓글을 삭제하시겠습니까?");
		
		var params = "replyNo=" + vr_no + "&v_no=" + v_no;	
		if(result) {
			$.ajax({
				url: "videoReplyDelete.ajax",
				type: "post",
				data: params,
				dataType: "json",
				success: function(resultData, status, xhr) {			
