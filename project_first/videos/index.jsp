...
<%
	...
	if (listCount > 0) {
		if (!searchOption) {
			videoList = dao.videoList(startRow, endRow);
		} else {
...
<script type="text/javascript">
	function noLoginupload() {
		alert("영상 업로드를 하시려면 로그인이 필요합니다.");
		location.href="mem/mem_login.jsp";
	}
</script>
...
<article class="">
			...
			<c:if test="${empty videoList }">
				...
				<c:if test="${ !empty sessionScope.isLogin }">
					<input type="button" onclick="location.href='videoUploadForm'" value="동영상 업로드" class="btn btn-outline-primary">
				</c:if>
				<c:if test="${ empty sessionScope.isLogin }">
					<input type="button" onclick=noLoginupload() value="동영상 업로드" class="btn btn-outline-primary">
				</c:if>
			</c:if>
			<c:if test="${!searchOption and !empty videoList }">
				<c:forEach var="v" items="${videoList}">
